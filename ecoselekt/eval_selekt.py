...
def eval_selekt(project_name):
    ...
    for i in range(settings.MODEL_HISTORY, len(windows) - settings.C_TEST_WINDOWS):
        ...
        for x_split in range(i + 1, len(windows) - settings.C_TEST_WINDOWS + 1):
            if settings.TEST_SIZE < settings.SHIFT:
                split = windows[x_split][-settings.TEST_SIZE :]
            else:
                split = pd.concat(
                    [
                    ...
                    split = pd.concat(
                        [
                            ...
                            windows[x_split + settings.F_TEST_WINDOWS][
                                -settings.SHIFT : (settings.TEST_SIZE % settings.SHIFT)
                                - settings.SHIFT
                            ],
                        ],
                        ignore_index=True,
    ...
    evaluation_df.to_csv(
        settings.DATA_DIR / f"{settings.EXP_ID}_{project_name}_selekt_eval.csv", index=False
    )
