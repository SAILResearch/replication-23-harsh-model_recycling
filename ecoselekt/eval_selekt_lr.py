...
def eval_selekt(project_name):
    ...
    selekt_pred_df = pd.read_csv(
        settings.DATA_DIR / f"{settings.EXP_ID}_{project_name}_selekt_pred_lr.csv"
    )
    ...
    for i in range(settings.MODEL_HISTORY, len(windows) - settings.C_TEST_WINDOWS):
        ...
        RF_result = RF_result.drop(columns=["window", "y_model_pred_proba", "y_model_pred"])
    ...
    evaluation_df.to_csv(
        settings.DATA_DIR / f"{settings.EXP_ID}_{project_name}_selekt_eval_lr.csv", index=False
    )
