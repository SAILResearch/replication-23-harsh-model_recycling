...
def inference_selekt(project_name):
    ...
    for i in range(settings.MODEL_HISTORY, len(windows) - settings.C_TEST_WINDOWS):
        ...
        with open(
            settings.MODELS_DIR / f"{settings.EXP_ID}_{project_name}_w{i}_best_old_model_lr.pkl",
            "rb",
        ) as f:
        ...
        latest_nn = load_model(i)
        # average probability of latest model and best model
        perf_df["y_pred_proba_eco"] = (
            old_nn.predict_proba(test_feature)[:, 1] + latest_nn.predict_proba(test_feature)[:, 1]
        ) / 2
        # assign prediction based on average probability
        perf_df["y_pred_eco"] = perf_df["y_pred_proba_eco"].apply(lambda x: 1 if x >= 0.5 else 0)
