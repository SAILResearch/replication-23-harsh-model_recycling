...
def inference_selekt(project_name):
    ...
    for i in range(settings.MODEL_HISTORY, len(windows) - settings.C_TEST_WINDOWS):
        ...
        _, past_commit_id, _ = get_combined_df_lr(
            past_split.code,
        ...
        with open(
            settings.MODELS_DIR / f"{settings.EXP_ID}_{project_name}_w{i}_knn_lr.pkl",
            "rb",
        ) as f:
            knn = pickle.load(f)
        ...
        start = time.time()
        with open(
            settings.MODELS_DIR / f"{settings.EXP_ID}_{project_name}_w{i}_preprocess_knn_lr.pkl",
            "rb",
        ) as f:
