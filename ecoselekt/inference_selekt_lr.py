...
def inference_selekt(project_name):
    ...
    for i in range(settings.MODEL_HISTORY, len(windows) - settings.C_TEST_WINDOWS):
        ...
        test_feature, test_commit_id, new_test_label = get_combined_df_lr(
            split.code,
            split.commit_id,
            split.label,
        ...
        with open(
            settings.MODELS_DIR / f"{settings.EXP_ID}_{project_name}_w{i}_stat_models_lr.pkl",
            "rb",
        ) as f:
        ...
        with open(
            settings.MODELS_DIR / f"{settings.EXP_ID}_{project_name}_w{i}_preprocess_lr.pkl", "rb"
        ) as f:
        ...
        start = time.time()
        with open(
            settings.MODELS_DIR / f"{settings.EXP_ID}_{project_name}_w{i}_selekt_model_lr.pkl", "rb"
        ) as f:
            nn = pickle.load(f)
        _LOGGER.info(f"Loaded selekt model in {time.time() - start}")

        final_test_feature = np.concatenate(
            [
                preprocess_.transform(test_feature).toarray(),
        ...
        perf_df["y_pred_eco"] = nn.predict(final_test_feature)
        perf_df["y_pred_proba_eco"] = nn.predict_proba(final_test_feature)[:, 1]
        ...
        selekt_pred_df.to_csv(
            settings.DATA_DIR / f"{settings.EXP_ID}_{project_name}_selekt_pred_lr.csv", index=False
        )
