...
def inference_selekt(project_name):
    ...
    for i in range(settings.MODEL_HISTORY, len(windows) - settings.C_TEST_WINDOWS):
        ...
        with open(
            settings.MODELS_DIR / f"{settings.EXP_ID}_{project_name}_w{i}_knn_nn.pkl",
            "rb",
        ) as f:
            knn = pickle.load(f)
        ...
        start = time.time()
        with open(
            settings.MODELS_DIR / f"{settings.EXP_ID}_{project_name}_w{i}_preprocess_knn_nn.pkl",
            "rb",
        ) as f:
        ...
        pred_df = pred_df[
            pred_df["model_version"].isin(perf_df["y_model_pred"].unique())
        ].reset_index(drop=True)

        # add models probabilities as features
        for model_version in perf_df["y_model_pred"].unique():
            prob_df = (
        ...
        for best_model in perf_df["y_model_pred"].unique():
            best_model_pred = pred_df.loc[perf_df["y_model_pred"] == best_model][
                f"prob_{best_model}"
            ].values
            perf_df.loc[perf_df["y_model_pred"] == best_model, "y_pred_eco"] = np.where(
                best_model_pred > 0.5, 1, 0
            )
            perf_df.loc[perf_df["y_model_pred"] == best_model, "y_pred_proba_eco"] = best_model_pred
