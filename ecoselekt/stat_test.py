...
def calc_cliffs_delta(project_name="activemq"):
    # load baseline evaluation results
    base_eval_df = pd.read_csv(
        settings.DATA_DIR / f"{settings.EXP_ID}_{project_name}_base_eval.csv"
    )
...
def main():
    ...
    try:
        ...
        for project_name in settings.PROJECTS:
            ...
            d_prec, d_rec, d_f1, d_auc, d_gmean, d_ap, d_spec = calc_cliffs_delta(project_name)
        ...
        cliff_df.to_csv(settings.DATA_DIR / f"{settings.EXP_ID}_cliff.csv", index=False)
    except Exception:
