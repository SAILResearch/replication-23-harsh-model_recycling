...
def get_combined_df_lr(code_commit, commit_id, label, metrics_df):
    ...
    # drop arexp for high correlation
    metrics_df = metrics_df.drop(["arexp"], axis=1)
...
def save_model_ckpts(project_name="activemq"):
    ...
    for i, window in enumerate(windows[: -settings.C_TEST_WINDOWS]):
        ...
        full_feature, full_commit_id, full_test_label = get_combined_df_lr(
            full_split.code,
        ...
        pipeline = make_pipeline_imb(
            ...
            LogisticRegression(random_state=settings.RANDOM_SEED, n_jobs=-1),
        )
        ...
        pickle.dump(
            clf,
            open(
                settings.MODELS_DIR / f"{settings.EXP_ID}_{project_name}_w{i}_model_lr.pkl",
                "wb",
            ),
        )
