...
def save_selekt_model(project_name="activemq"):
    ...
    for i in range(settings.MODEL_HISTORY, len(windows) - settings.C_TEST_WINDOWS):
        ...
        pred_df = pred_df.sort_values("error").drop_duplicates(subset="commit_id", keep="first")
        rank_to_model = {}
        # keep the most frequent model version for each rank group
        for rank in pred_df["rank"].unique():
            most_freq_model_version = (
                pred_df[pred_df["rank"] == rank]["model_version"].value_counts().index[0]
            )
            rank_to_model[rank] = most_freq_model_version
            pred_df["model_version"] = np.where(
                (pred_df["rank"] == rank) & (pred_df["model_version"] != most_freq_model_version),
                most_freq_model_version,
        ...
        model_versions = set(pred_df["model_version"].unique())
        ...
        estimators = [load_model(model_version) for model_version in sorted(model_versions)]

        clf = EnsembleVoteClassifier(
            clfs=estimators,
            voting="soft",
