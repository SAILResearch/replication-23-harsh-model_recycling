...
def save_selekt_model(project_name="activemq"):
    ...
    for i in range(settings.MODEL_HISTORY, len(windows) - settings.C_TEST_WINDOWS):
        ...
        _LOGGER.info(f"Max f1 score: {max_score} for model version: {max_score_model_version}")

        pred_df = pred_df[pred_df["model_version"].isin([max_score_model_version])]

        with open(
            settings.MODELS_DIR / f"{settings.EXP_ID}_{project_name}_w{i}_best_old_model.pkl", "wb"
        ) as f:
