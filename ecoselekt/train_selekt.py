...
def objective_func(k, train_feature, train_label, valid_feature, valid_label, estimators):
...
def save_selekt_model(project_name="activemq"):
    ...
    for i in range(settings.MODEL_HISTORY, len(windows) - settings.C_TEST_WINDOWS):
        ...
        _LOGGER.info(f"Model versions removed: {models_to_remove}")

        with open(
            settings.MODELS_DIR / f"{settings.EXP_ID}_{project_name}_w{i}_wilcoxon_models.pkl", "wb"
        ) as f:
