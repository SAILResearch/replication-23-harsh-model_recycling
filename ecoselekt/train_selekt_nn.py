...
def save_selekt_model(project_name="activemq"):
    ...
    for i in range(settings.MODEL_HISTORY, len(windows) - settings.C_TEST_WINDOWS):
        ...
        # stratified random resampling to balance the dataset and reduce to `settings.WINDOW_SIZE`
        ...
        prob = clf.predict_proba(pred_df[model_prob_features].values)
        pred = clf.predict(pred_df[model_prob_features].values)
